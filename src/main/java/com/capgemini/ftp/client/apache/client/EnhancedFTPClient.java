/*
 * Derived from the Apache FTP Client under the Apache LICENSE 2.0.
 *
 ...
 */
package com.capgemini.ftp.client.apache.client;
...
/**
 * The responsibility of this class to to enhance the basic 
 * Apache FTPClient class with functionality necessary to support operation
 * within the online HMRC estate.
 * <p>
 * The original Apache FTPS class is not particularly friendly to subclasses
 * when exposing private attributes that subclasses will need to have access to
 * when overriding protected or public methods. This enhanced FTPClient class
 * provides access to the most essential of these properties.
 * <p>
 * EnhancedFTPClient encapsulates all the functionality necessary to store and
 ...
 * <pre>
    EnhancedFTPClient ftp = new EnhancedFTPClient();
 ...
 * <p>
 ...
 FTP command methods in EnhancedFTPClient is such that they either return a
 ...
 * <p>
 The default settings for EnhancedFTPClient are for it to use
 ...
 * <code> FTP.BINARY_FILE_TYPE </code>.  Because there are at least 4
 ...
 them around the streams returned or required by the EnhancedFTPClient methods.
 EnhancedFTPClient uses the {@link ToNetASCIIOutputStream NetASCII}
 ...
 * <p>
 ...
 significantly, the EnhancedFTPClient automatically issues a new PORT (or EPRT) command
 ...
 * <p>
 ...
 given time period (usually 900 seconds).  The EnhancedFTPClient class will detect a
 ...
 * {@link #disconnect  disconnect() } to properly clean up the
 system resources used by EnhancedFTPClient.  Before disconnecting, you may check the
 ...
 * <pre>
    EnhancedFTPClient f = new EnhancedFTPClient();
    f.connect(server);
 ...
 * <p>
 For examples of using EnhancedFTPClient on servers whose directory listings
 <ul>
 ...
 */
public class EnhancedFTPClient extends FTP
implements Configurable
{
    
	private static final Logger logger = LoggerFactory.getLogger(EnhancedFTPClient.class);
     ...
     * This is the default data connection mode when and EnhancedFTPClient instance
 is created.
     */
    ...
    /** 
     * The remote host name or IP address on which the connection was originally
     * made.
     */
    private String __connectionRemoteHost;    
    ...
    private boolean __skipIpFromPasvReply = false;
    ...
    private static class PropertiesSingleton {
        ...
        static {
            InputStream resourceAsStream = EnhancedFTPClient.class.getResourceAsStream(SYSTEM_TYPE_PROPERTIES);
            Properties p = null;
    ...
    throws MalformedServerReplyException
    {
        ...
        if (isSkipIpFromPasvReply()) {
            __passiveHost = getConnectionRemoteHost();
            logger.debug("Skipping host in PASV reply. Setting passiveHost to: {}", getConnectionRemoteHost());
        } else {
            __passiveHost = m.group(1).replace(',', '.'); // Fix up to look like IP address
            logger.debug("Using host from PASV reply. Setting passiveHost to: {}", __passiveHost);
        }
        ...
        if (__passiveNatWorkaround) {
            try {
                logger.debug("Using passiveNatWorkaround...");
                ...
                if (host.isSiteLocalAddress()) {
                    ...
                    if (!remote.isSiteLocalAddress()){
                        ...
                        logger.debug("Passive NAT workaround has set passiveHost to: {}", hostAddress);
    ...
    throws IOException
    {
        ...
        { // We must be in PASSIVE_LOCAL_DATA_CONNECTION_MODE
            ...
            if (__receiveDataSocketBufferSize > 0) {
                logger.debug("Setting receiveDataSocketBufferSize to: {}", __receiveDataSocketBufferSize);                
                socket.setReceiveBufferSize(__receiveDataSocketBufferSize);
            }
            if (__sendDataSocketBufferSize > 0) {
                logger.debug("Setting sendDataSocketBufferSize to: {}", __sendDataSocketBufferSize);                
                socket.setSendBufferSize(__sendDataSocketBufferSize);
            }
            if (__passiveLocalHost != null) {
                logger.debug("Binding socket to passiveLocalHost: {}", __passiveLocalHost);
            ...
            logger.debug("Connecting to host: {} on port {} ", __passiveHost, __passivePort);
 ...
 EnhancedFTPClient is currently connected or not.
 <p>
 ...
 data transfers.  The EnhancedFTPClient will stay in
 ...
 before EVERY server to server transfer attempt.  The EnhancedFTPClient will
 ...
 This is because EnhancedFTPClient sends a PASV command to the server only
 ...
 to it by another EnhancedFTPClient.
 <p>
     ...
     * The other server must have had a <code> remoteRetrieve </code> issued
 to it by another EnhancedFTPClient.  Many FTP servers require that a base
 ...
 those servers use the other version of <code> remoteStoreUnique</code>
     ...
     * <code> remoteRetrieve </code> issued to it by another EnhancedFTPClient.
 <p>
    ...
    /**
     * There are a few EnhancedFTPClient methods that do not complete the
 ...
 FTP server.  EnhancedFTPClient will remember the value and return the
 ...
 <p>
     ...
     * <p>
     * <pre>
    EnhancedFTPClient f=EnhancedFTPClient();
    f.connect(server);
    f.login(username, password);
    FTPListParseEngine engine = f.initiateListParsing(directory);
 ...
 in its response to the FEAT command, which may be retrieved by EnhancedFTPClient.features()
     *
...
//    @Override
    ...
    private static class CSL implements CopyStreamListener {
        ...
        private final EnhancedFTPClient parent;
        ...
        CSL(EnhancedFTPClient parent, long idleTime, int maxWait) throws SocketException {
            this.idle = idleTime;
            this.parent = parent;
...
//        @Override
    ...
    /**
     * The IP address sent by the passive host can be ignored and overridden
     * with the original IP address used to make the connection.
     * 
     * @param enabled 
     */
    public void setSkipIpFromPasvReply(boolean enabled) {
        logger.debug("Setting skipIPFromPasvReply to {}", enabled);
        __skipIpFromPasvReply = enabled;
    }
    
    /**
     * 
     * @return
     *  <code>true</code> if the IP address from the server's PASV reply should
     *  be ignored. The original IP address used for the connection will be used
     *  instead. Please see
     * {@link com.capgemini.ftp.client.apache.client.EnhancedFTPClient#getConnectionRemoteHost getConnectionRemoteHost }
     * for further details.
     */
    public boolean isSkipIpFromPasvReply() {
        return __skipIpFromPasvReply;
    }

        
    /**
     * @return
     *  The remote hostname or IP address on which the connection was originally
     *  made.
     */
    protected String getConnectionRemoteHost() {
        return __connectionRemoteHost;
    }   
    
    @Override
    public void connect(String hostname) throws SocketException, IOException {
        this.__connectionRemoteHost = hostname;
        super.connect(hostname); 
    }

    @Override
    public void connect(InetAddress host) throws SocketException, IOException {
        this.__connectionRemoteHost = host.getHostAddress();
        super.connect(host); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void connect(String hostname, int port, InetAddress localAddr, int localPort) throws SocketException, IOException {
        this.__connectionRemoteHost = hostname;
        super.connect(hostname, port, localAddr, localPort); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void connect(InetAddress host, int port, InetAddress localAddr, int localPort) throws SocketException, IOException {
        this.__connectionRemoteHost = host.getHostAddress();
        super.connect(host, port, localAddr, localPort); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void connect(String hostname, int port) throws SocketException, IOException {
        this.__connectionRemoteHost = hostname;
        super.connect(hostname, port); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void connect(InetAddress host, int port) throws SocketException, IOException {
        this.__connectionRemoteHost = host.getHostAddress();
        super.connect(host, port); //To change body of generated methods, choose Tools | Templates.
    }
